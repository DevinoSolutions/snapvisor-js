...
export const projectOption = new Option(
  "--project <slug>",
  "Snapvisor project slug (account/project), used to disambiguate tokenless authentication when multiple projects are linked to the same repository",
);

export type BuildNameOption = { buildName?: string | undefined };
export const buildNameOption = new Option(
  "--build-name <string>",
  "Name of the build, in case you want to run multiple Snapvisor builds in a single CI job",
);
